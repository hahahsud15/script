...
local updateLog = Instance.new("TextLabel", loadFrame)
updateLog.Size = UDim2.new(1,-20,0,80)
updateLog.Position = UDim2.new(0,10,0,100)
updateLog.BackgroundTransparency = 1
updateLog.Text = "📝 V15 UPDATE LOG:\n✅ NEW: Click Teleport - Click anywhere to TP!\n✅ FIX: Live Ping System 🟢🟡🔴\n✅ FIX: Anti-AFK + Auto-Hop\n✅ 100% V15 Rebrand!"
updateLog.TextColor3 = Color3.fromRGB(0,255,150)
...
local function buildMain(lang)
    ...
    makeToggle(pTab, "FLIEGEN - JOYSTICK + WASD + UP/DOWN HANDY","FLY - JOYSTICK + WASD + UP/DOWN MOBILE", function(on)
        if on then
            ...
            local hrp=lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")
            if hrp then if hrp:FindFirstChild("V14Fly") then hrp.V14Fly:Destroy() end if hrp:FindFirstChild("V15Gyro") then hrp.V15Gyro:Destroy() end end
        end
    end)
