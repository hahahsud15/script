-- V9 FINAL FIXED - OPEN GEHT 100% - DRAG FIX 2026 - hahahsud15
...
if parent:FindFirstChild("V9_FINAL") then pcall(function() parent.V9_FINAL:Destroy() end) task.wait(0.2) end
...
main.Name="Main"
main.Size=UDim2.new(0.94,0,0.90,0)
main.Position=UDim2.new(0.03,0,0.05,0)
main.BackgroundColor3=Color3.fromRGB(14,14,20)
main.BorderSizePixel=0
main.Active=true
main.Draggable=true
main.Visible=false -- Am Anfang zu
...
local title=Instance.new("TextLabel", top) title.Size=UDim2.new(0.65,0,1,0) title.Position=UDim2.new(0.03,0,0,0) title.Text="V9 FINAL - OPEN FIXED" title.BackgroundTransparency=1 title.TextColor3=Color3.fromRGB(0,0,0) title.TextScaled=true title.Font=Enum.Font.GothamBlack title.TextXAlignment=Enum.TextXAlignment.Left
...
-- RIESEN OPEN BUTTON - FIXED OHNE DRAGGABLE BUG - 2026 METHODE!
local openBtn=Instance.new("TextButton", gui)
openBtn.Name="OpenBtn"
openBtn.Size=UDim2.new(0,180,0,180) -- NOCH GRÖSSER!
openBtn.Position=UDim2.new(0.5,-90,0.5,-90)
openBtn.Text="V9\nOPEN\n👆\nDRÜCK!"
...
-- WICHTIG: KEIN Draggable = true! Das war der Bug!
Instance.new("UICorner", openBtn).CornerRadius=UDim.new(0,35)
local stroke=Instance.new("UIStroke", openBtn) stroke.Color=Color3.new(1,1,1) stroke.Thickness=4

-- CUSTOM DRAG SYSTEM - WIE TIKTOK/YOUTUBE 2026 PROS!
...
openBtn.InputChanged:Connect(function(input)
    if dragging and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then
        ...
        if delta.Magnitude>10 then dragMoved=true end
    ...
    if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
        if dragging then
            ...
            if not dragMoved then
                -- War ein Click, kein Drag! Dann öffnen!
                ...
                if main.Visible then
                    ...
                    openBtn.Size=UDim2.new(0,180,0,180)
                    openBtn.Text="V9\nOPEN\n👆\nDRÜCK!"
                    openBtn.Position=UDim2.new(0.5,-90,0.5,-90)
                end
            end
...
-- FIXED TOGGLES - GANZE REIHE = 1 BUTTON!
function makeSection(txt)
    local f=Instance.new("Frame", scroll) f.Size=UDim2.new(1,0,0,36) f.BackgroundColor3=Color3.fromRGB(30,30,38) Instance.new("UICorner", f).CornerRadius=UDim.new(0,10)
...
function makeBtn(txt, color, cb)
    local b=Instance.new("TextButton", scroll) b.Size=UDim2.new(1,0,0,65) b.Text=txt b.BackgroundColor3=color b.TextColor3=Color3.new(1,1,1) b.TextScaled=true b.Font=Enum.Font.GothamBold b.BorderSizePixel=0 Instance.new("UICorner", b).CornerRadius=UDim.new(0,12) b.AutoButtonColor=false
    b.Activated:Connect(function() pcall(cb) end)
end

-- LOGIC - ALLES MIT PCALL
...
local function startFly()
    pcall(function()
        ...
        if hrp:FindFirstChild("V9FlyAtt") then hrp.V9FlyAtt:Destroy() end
        if hrp:FindFirstChild("V9FlyLV") then hrp.V9FlyLV:Destroy() end
        flyAtt=Instance.new("Attachment") flyAtt.Name="V9FlyAtt" flyAtt.Parent=hrp
        flyLV=Instance.new("LinearVelocity") flyLV.Name="V9FlyLV" flyLV.Attachment0=flyAtt flyLV.MaxForce=math.huge flyLV.VectorVelocity=Vector3.new(0,0,0) flyLV.Parent=hrp
        task.spawn(function()
            while flyOn and flyLV and flyLV.Parent do
                pcall(function()
                    ...
                    if move.Magnitude>0 then dir=cam:VectorToWorldSpace(Vector3.new(move.X,0,move.Z)) dir=Vector3.new(dir.X,0,dir.Z) if dir.Magnitude>0 then dir=dir.Unit*move.Magnitude end end
                    if flyUp then dir=dir+Vector3.new(0,1.3,0) end
                    if flyDown then dir=dir+Vector3.new(0,-1.3,0) end
...
-- GUI - EXTRA LANG!
makeSection("1. BEWEGUNG - ALLES GEHT - GANZE REIHE DRÜCKEN!")
makeToggle("✈️ FLY - Joystick bewegen!", Color3.fromRGB(0,255,140), function(on) flyOn=on if on then startFly() else pcall(function() if flyLV then flyLV:Destroy() end if flyAtt then flyAtt:Destroy() end end) end end)
makeToggle("⬆️ FLY HOCH HALTEN", Color3.fromRGB(0,140,255), function(on) flyUp=on end)
makeToggle("⬇️ FLY RUNTER HALTEN", Color3.fromRGB(0,100,200), function(on) flyDown=on end)
makeToggle("👻 NOCLIP - Durch Wände", Color3.fromRGB(150,0,255), function(on) noclipOn=on end)
makeToggle("♾️ INF JUMP HOLD - HALTEN=HOCH!", Color3.fromRGB(0,255,100), function(on) if on then infHoldConn=RS.Heartbeat:Connect(function() pcall(function() if LP.Character then local h=LP.Character.Humanoid if h:GetState()==Enum.HumanoidStateType.Jumping or UIS:IsKeyDown(Enum.KeyCode.Space) or h.Jump then h:ChangeState(Enum.HumanoidStateType.Jumping) LP.Character.HumanoidRootPart.Velocity=Vector3.new(LP.Character.HumanoidRootPart.Velocity.X,75,LP.Character.HumanoidRootPart.Velocity.Z) end end end) end) else if infHoldConn then infHoldConn:Disconnect() end end end)
...
makeBtn("↗️ LONG JUMP", Color3.fromRGB(255,100,0), function() if LP.Character then local cf=workspace.CurrentCamera.CFrame LP.Character.HumanoidRootPart.Velocity=Vector3.new(cf.LookVector.X*130,60,cf.LookVector.Z*130) end end)

makeSection("2. SEHEN - ALLES GEHT")
makeToggle("👁️ ESP WALLHACK", Color3.fromRGB(255,0,80), function(on) if on then for _,p in pairs(Players:GetPlayers()) do if p~=LP and p.Character and not p.Character:FindFirstChild("V9ESP") then local hl=Instance.new("Highlight", p.Character) hl.Name="V9ESP" hl.FillColor=Color3.fromRGB(255,0,60) hl.OutlineColor=Color3.fromRGB(0,255,140) hl.FillTransparency=0.35 hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop end end else for _,p in pairs(Players:GetPlayers()) do if p.Character then for _,v in pairs(p.Character:GetChildren()) do if v.Name=="V9ESP" then v:Destroy() end end end end end end)
...
makeBtn("☀️ FULLBRIGHT TAG", Color3.fromRGB(255,240,0), function() Lighting.Brightness=4 Lighting.ClockTime=13 Lighting.FogEnd=100000 Lighting.GlobalShadows=false end)
...
makeBtn("🛡️ ANTI-AFK", Color3.fromRGB(0,150,60), function() local vu=game:GetService("VirtualUser") LP.Idled:Connect(function() vu:Button2Down(Vector2.new(0,0),workspace.CurrentCamera.CFrame) wait(1) vu:Button2Up(Vector2.new(0,0),workspace.CurrentCamera.CFrame) end) end)
...
print("✅ V9 FINAL GELADEN - OPEN BUTTON FIXED - DRAG FIX 2026!")
