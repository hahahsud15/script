...
local function createStolenWatermark()
    pcall(function()
        local parent = game.CoreGui or game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
...
if not checkWasserzeichen() then createStolenWatermark() error("STOLEN FROM hahahsud15 - V15") return end
print("V15 ULTRA OP by hahahsud15 | Anti-Steal V3 Active")
...
local LANG = getgenv().V15_LANG or nil
...
gui.Name = "V15_ENHANCED_"..getgenv().V15_OWNER
...
loadTitle.Text = "V15 ENHANCED\n6 NEW OP FEATURES!\nby "..getgenv().V15_OWNER.." 🔥"
...
langTitle2.Text = "V15 ENHANCED - LANGUAGE SELECTION"
...
OPEN"
    ...
    title.Text = "  V15 ULTRA OP - 6 NEW FEATURES | "..getgenv().V15_OWNER.." 🔥"..getgenv().V15_OWNER.." 🔥"
    ...
    perfLabel.Text = "🟢 PING: 0ms | FPS: 0 | PLAYERS: 0 | V15 READY ✅"
    ...
    RunService.RenderStepped:Connect(function(dt)
        if not checkWasserzeichen() then return end
        updatePing()
        local fps = math.clamp(math.floor(1/dt), 0, 999)
        local pStatus, pMs = getPingStatus()
        -- FIXED: Zeigt nur DEINEN echten Ping/FPS, nicht von anderen!
        perfLabel.Text = " "..pStatus.." PING: "..pMs.."ms (DU) | FPS: "..fps.." (DU) | PLAYERS: "..#Players:GetPlayers().." | V15 ✅"
        autoHopTick()
    end)
    ...
    -- ===== V15 NEW OP FEATURES - 100% WORKING =====
    ...
    local function toggleClickTP(on)
        ...
        if on then
            ...
            clickTPConn = mouse.Button1Down:Connect(function()
                ...
                if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or isMobile or UserInputService:IsKeyDown(Enum.KeyCode.Q) then
                    local char = lp.Character
                    if char and char:FindFirstChild("HumanoidRootPart") then
                        char.HumanoidRootPart.CFrame = CFrame.new(mouse.Hit.Position + Vector3.new(0,3,0))
                        game.StarterGui:SetCore("SendNotification",{Title="📍 CLICK TP", Text=T("Teleportiert!","Teleported!"), Duration=1})
                    end
                end
            end)
            game.StarterGui:SetCore("SendNotification",{Title="📍 CLICK TP", Text=T("AN! Halte CTRL/Shift/Q + Klick auf Boden!","ON! Hold CTRL/Shift/Q + Click ground!"), Duration=3})
        else
    ...
    local function toggleAntiFling(on)
        ...
        if on then
                        ...
                        if plr ~= lp and plr.Character then
                            for _,part in pairs(plr.Character:GetDescendants()) do
                                if part:IsA("BasePart") then
                                    ...
                                    if part.AssemblyLinearVelocity.Magnitude > 60 or part.AssemblyAngularVelocity.Magnitude > 60 then
            ...
            table.insert(antiFlingConns, RunService.Stepped:Connect(antiFlingLoop))
            table.insert(antiFlingConns, RunService.Heartbeat:Connect(antiFlingLoop))
            game.StarterGui:SetCore("SendNotification",{Title="🛡️ ANTI-FLING", Text=T("GOD MODE AKTIV! Du bist unflingbar!","GOD MODE ACTIVE! You are unflingable!"), Duration=2})
        else
            for _,c in pairs(antiFlingConns) do pcall(function() c:Disconnect() end) end
            antiFlingConns = {}
        end
    end
    ...
    local function toggleFreecam(on)
        ...
        if on then
            ...
            freecamConn = RunService.RenderStepped:Connect(function()
                ...
                if UserInputService:IsKeyDown(Enum.KeyCode.W) then move += freecamPos.LookVector * speed/60 end
                if UserInputService:IsKeyDown(Enum.KeyCode.S) then move -= freecamPos.LookVector * speed/60 end
                if UserInputService:IsKeyDown(Enum.KeyCode.A) then move -= freecamPos.RightVector * speed/60 end
                if UserInputService:IsKeyDown(Enum.KeyCode.D) then move += freecamPos.RightVector * speed/60 end
                if UserInputService:IsKeyDown(Enum.KeyCode.Q) or UserInputService:IsKeyDown(Enum.KeyCode.Space) then move += Vector3.new(0,speed/60,0) end
                if UserInputService:IsKeyDown(Enum.KeyCode.E) or UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) then move -= Vector3.new(0,speed/60,0) end
                if isMobile and lp.Character then
                    local hum = lp.Character:FindFirstChildOfClass("Humanoid")
                    if hum and hum.MoveDirection.Magnitude > 0 then
                        local md = hum.MoveDirection
                        move += Vector3.new(md.X,0,md.Z) * speed/60
                    end
                end
                freecamPos = freecamPos + move
                cam.CFrame = freecamPos
            end)
            game.StarterGui:SetCore("SendNotification",{Title="📷 FREECAM", Text=T("AN! WASD+Q/E, Character bleibt stehen! Perfekt zum Spyen!","ON! WASD+Q/E, character stays! Perfect for spying!"), Duration=3})
        else
    ...
    local function toggleInfZoom(on)
        if on then
            lp.CameraMaxZoomDistance = 10000
            lp.CameraMinZoomDistance = 0.5
            game.StarterGui:SetCore("SendNotification",{Title="🔭 INF ZOOM", Text=T("10000 Studs Zoom AKTIV!","10000 Studs Zoom ACTIVE!"), Duration=2})
        else
    ...
    local function createTouchJoystick()
        ...
        sg.Name = "TouchFlyJoystickV15"
        joystickGui = sg
        local back = Instance.new("Frame", sg)
        back.Size = UDim2.new(0,120,0,120)
        back.Position = UDim2.new(0,20,1,-170)
        back.BackgroundColor3 = Color3.fromRGB(0,0,0)
        back.BackgroundTransparency = 0.5
        Instance.new("UICorner", back).CornerRadius = UDim.new(0,60)
        Instance.new("UIStroke", back).Color = Color3.fromRGB(0,255,150)
        local label = Instance.new("TextLabel", back)
        ...
        label.TextSize = 10
        local stick = Instance.new("Frame", back)
        stick.Name = "Stick"
        stick.Size = UDim2.new(0,50,0,50)
        stick.Position = UDim2.new(0.5,-25,0.5,-25)
        stick.BackgroundColor3 = Color3.fromRGB(0,255,150)
        Instance.new("UICorner", stick).CornerRadius = UDim.new(0,25)
        local upBtn = Instance.new("TextButton", sg)
        upBtn.Size = UDim2.new(0,60,0,60)
        upBtn.Position = UDim2.new(1,-80,1,-230)
        upBtn.Text = "⬆️ UP"
        upBtn.BackgroundColor3 = Color3.fromRGB(0,150,0)
        upBtn.TextColor3 = Color3.new(1,1,1)
        upBtn.Font = Enum.Font.GothamBold
        Instance.new("UICorner", upBtn).CornerRadius = UDim.new(0,15)
        local downBtn = Instance.new("TextButton", sg)
        downBtn.Size = UDim2.new(0,60,0,60)
        downBtn.Position = UDim2.new(1,-80,1,-160)
        downBtn.Text = "⬇️ DOWN"
        ...
        local dragStart = nil
        local stickStart = nil
        stick.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                dragging = true
                dragStart = input.Position
                stickStart = stick.Position
            end
        end)
        UserInputService.InputChanged:Connect(function(input)
            if dragging and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
                local delta = input.Position - dragStart
                local newX = math.clamp(stickStart.X.Offset + delta.X, -35, 35)
                local newY = math.clamp(stickStart.Y.Offset + delta.Y, -35, 35)
                stick.Position = UDim2.new(0, newX+35, 0, newY+35)
            end
        end)
    ...
    local function openServerScanner()
        local scanGui = Instance.new("ScreenGui", gui)
        scanGui.Name = "ServerScannerV15"
        local frame = Instance.new("Frame", scanGui)
        ...
        title.Text = "🔍 V15 SERVER SCANNER - by hahahsud15"
        ...
        closeBtn.MouseButton1Click:Connect(function() scanGui:Destroy() end)
        ...
        addLabel("📊 "..T("Spieler","Players")..": "..#Players:GetPlayers().." / "..Players.MaxPlayers, Color3.fromRGB(0,255,150))
        addLabel("📡 "..T("Dein Ping","Your Ping")..": "..currentPing.."ms "..pingStatus.." | FPS: "..math.floor(1/0.016).." (lokal)", Color3.fromRGB(0,255,255))
        addLabel("🕒 Server Alter: "..math.floor(workspace.DistributedGameTime/60).." min", Color3.new(1,1,1))
        addLabel("--- "..T("SPIELER LISTE (nur lokale Checks)","PLAYER LIST (local checks only)").." ---", Color3.fromRGB(255,255,0))
        local hackers = 0
        local laggers = 0
        for _,plr in pairs(Players:GetPlayers()) do
            ...
            pcall(function()
                if plr.Character and plr.Character:FindFirstChildOfClass("Humanoid") then
                    ...
                    if hum.WalkSpeed > 35 then isHacker=true reason="Speed "..hum.WalkSpeed end
                    if hum.JumpPower > 100 or hum.JumpHeight > 15 then isHacker=true reason=reason.." JumpHigh" end
                end
                if plr ~= lp then
                    -- We cannot get other players ping, show as unknown
                    pingFake = T("Unbekannt (Roblox API Limit)","Unknown (Roblox API Limit)")
                else
                    pingFake = currentPing.."ms"
                end
            end)
            if isHacker then hackers+=1 end
            local color = isHacker and Color3.fromRGB(255,50,50) or Color3.new(1,1,1)
            local txt = (isHacker and "🚨 [SUSPECT "..reason.."] " or "✅ ")..plr.Name.." | Ping: "..pingFake
            addLabel(txt, color)
        end
        addLabel("--- "..T("ERGEBNIS","RESULT").." ---", Color3.fromRGB(255,255,0))
        addLabel("🚨 "..T("Verdächtige","Suspects")..": "..hackers, hackers>0 and Color3.fromRGB(255,0,0) or Color3.fromRGB(0,255,0))
        addLabel(T("Hinweis: Roblox zeigt nur deinen eigenen Ping an!","Note: Roblox only shows your own ping!"), Color3.fromRGB(200,200,200))
        addLabel("© V15 by hahahsud15 - Anti-Steal V3 Active", Color3.fromRGB(0,255,150))
    end
    ...
    makeButton(sTab, "🔍 SERVER SCANNER - HACKER + PING CHECK","🔍 SERVER SCANNER - HACKER + PING CHECK", Color3.fromRGB(200,50,200), function()
        openServerScanner()
    end)
